...
class PCB {
	...
	constructor( name, options, layers, outlineLayer, unit ) {
		...
		let bounds = new GerberReader(
			...
			new ApertureStore( layers[ outlineLayer ].format ),
		...
		for( name in options.layers ) {
			...
			if( layers[ name ].type == 'gerber' ) {
				...
				if( ! options.layers[name].format ) {
					throw new Error( `Format must be specified for Excellon layer ${name}` );
				}
