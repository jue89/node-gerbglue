...
class ApertureStore {
	...
	add( job, reArr, srcFormat ) {
		...
		if( ! tmp ) throw new Error( "Unknown aperture definition: " + reArr[ 2 ] );
		...
		this._dict[ job ][ localApertureID ] = globalApertureID.toString();
	...
	lookup( job, localApertureID ) {
		...
		return this._dict[ job ][ localApertureID ];
...
module.exports = {
	ApertureStore: ApertureStore,
	GerberApertureAddRE: GerberApertureAddRE
};
