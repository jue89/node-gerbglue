...
const PlacerScoring = require( './placer-scoring.js' );
...
class Panel {
	...
	constructor( options ) {
		...
		if( options.placing.seperation == 'scoring' ) {
			this._placer = new PlacerScoring( options.placing, this._layers, this._unit );
		}
	...
	genPanel( placing ) {
		...
		// Place all PCBs
		let measurements = this._placer.place( placing, this._pcbs );

		// Create outline
		this._layers[ this._outlineLayer ].writer.addPolygon( this._unit, [
			{ x: 0,              y: 0              },
			{ x: measurements.x, y: 0              },
			{ x: measurements.x, y: measurements.y },
			{ x: 0,              y: measurements.y }
		] );

		// Write all layers into files
		for( let l in this._layers ) {
			fs.writeFileSync( `${this._dir}/${this._layers[l].file}`, this._layers[l].writer.toString() );
		}
